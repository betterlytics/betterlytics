...
use crate::db::{SessionReplayRow, SharedDatabase};
...
pub async fn presign_put_segment(
    ...
    let ip_address = crate::parse_ip(headers.clone())
        .unwrap_or(addr.ip())
        .to_string();
    ...
    if req.content_length == 0 || req.content_length > MAX_CONTENT_LENGTH_BYTES {
        return Err((
            ...
            "invalid content_length".to_string(),
        ));
    }
...
pub async fn finalize_session_replay(
    ...
    let started = chrono::DateTime::from_timestamp(req.started_at, 0)
        .ok_or((StatusCode::BAD_REQUEST, "invalid started_at".to_string()))?;
    let ended = chrono::DateTime::from_timestamp(req.ended_at, 0)
        .ok_or((StatusCode::BAD_REQUEST, "invalid ended_at".to_string()))?;
    ...
    meta.event_count = meta
        .event_count
        .saturating_add(req.event_count.unwrap_or_default());
