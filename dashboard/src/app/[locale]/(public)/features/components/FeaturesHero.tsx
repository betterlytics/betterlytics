...
export async function FeaturesHero() {
  const t = await getTranslations('public.features.hero');
  const demoDashboardPath = env.DEMO_DASHBOARD_ID ? `/share/${env.DEMO_DASHBOARD_ID}` : null;

  return (
    <section className='relative overflow-visible pt-20 pb-16 sm:pt-32 sm:pb-24'>
      <div className='relative container mx-auto px-4 sm:px-6 lg:px-8'>
        <div className='text-center'>
          ...
          <p className='text-muted-foreground mx-auto mb-10 max-w-3xl text-lg sm:text-xl'>{t('description')}</p>
