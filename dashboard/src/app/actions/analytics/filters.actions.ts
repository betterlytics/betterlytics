...
import { z } from 'zod';
import { FilterColumnSchema, type FilterColumn } from '@/entities/analytics/filter.entities';
...
export const getFilterOptionsAction = withDashboardAuthContext(
  async (ctx: AuthContext, query: BAAnalyticsQuery, params: z.infer<typeof FilterOptionsSchema>) => {
    ...
    const rows = await getDistinctValuesForFilterColumn(main, column as FilterColumn, search, limit);

    return column === 'device_type' ? toFormatted(rows, capitalizeFirstLetter) : rows;
  },
);

const GlobalPropertyKeysSchema = z.object({
  search: z.string().trim().max(128).optional(),
  limit: z.number().int().min(1).max(100).optional().default(50),
});

export const getGlobalPropertyKeysAction = withDashboardAuthContext(
  async (ctx: AuthContext, query: BAAnalyticsQuery, params: z.infer<typeof GlobalPropertyKeysSchema>) => {
    const { main } = toSiteQuery(ctx.siteId, query);
    const { search, limit } = GlobalPropertyKeysSchema.parse(params);
    return getAvailableGlobalPropertyKeys(main, search, limit);
  },
);
