...
import { z } from 'zod';
import { validateToken, disconnectPrisma } from '@/mcp/auth/token';
...
const token = process.env.BETTERLYTICS_TOKEN;
if (!token) {
  console.error('BETTERLYTICS_TOKEN environment variable is required');
  process.exit(1);
}
