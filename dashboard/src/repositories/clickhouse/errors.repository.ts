...
export async function getSessionTrailEvents(siteId: string, sessionId: string): Promise<SessionTrailEvent[]> {
  const query = safeSql`
    SELECT
      timestamp,
      event_type,
  ...
  return result.map((row) =>
    SessionTrailEventSchema.parse({
      ...row,
      timestamp: parseClickHouseDate(row.timestamp),
    }),
  );
}
