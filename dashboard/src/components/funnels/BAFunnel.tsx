  ...
  return (
    <div className='text-sm font-semibold'>
      ...
      {funnel.steps.map((step, index) => {
        const formattedFilter = formatQueryFilter(step.queryFilter, tFilters);
        return (
          <div
            key={`${step.queryFilter.column}-${step.queryFilter.operator}-${step.queryFilter.value}-${index}`}
          ...
          >
            <div className='flex items-end justify-between'>
              <div className='flex items-center gap-3'>
                ...
                <p className='truncate' title={formattedFilter}>
                  {formattedFilter}
                </p>
              </div>
                ...
                <>
                  ...
                  <div className='mt-0.5 flex w-full items-end justify-end'>
                    <p className='text-muted-foreground text-xs whitespace-nowrap'>
                      {step.dropoffCount} {t('dropped')} ({' '}
                      {formatPercentage(Math.floor(100 * step.dropoffRatio), 0)} )
                    </p>
                  </div>
                </>
              )}
