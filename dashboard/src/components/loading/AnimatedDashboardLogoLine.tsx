...
interface AnimatedDashboardLogoLineProps {
  size?: number;
  className?: string;
  speed?: number;
}
...
const TOTAL_CYCLE = LINE_TOTAL + 500; // Small gap before restart

// Line graph points - starts outside left, angled to match flow into circle 2
const LINE_POINTS = [
  { x: 0, y: 680 },     // Start: left edge, angled to flow into circle 2
...
export const AnimatedDashboardLogoLine = memo(function AnimatedDashboardLogoLine({
  size = 120,
  className = '',
  speed = 1,
}: AnimatedDashboardLogoLineProps) {
  ...
  const animate = useCallback((time: number) => {
    ...
    if (cycleTime < LINE_DRAW_DURATION) {
      // Drawing phase - segment by segment with pauses
      const segmentLengths = [
        SEG1 / LINE_LENGTH,
        SEG2 / LINE_LENGTH,
        (LINE_LENGTH - SEG1 - SEG2) / LINE_LENGTH,
      ];
      ...
      setLineOpacity(1);
    } else if (cycleTime < LINE_DRAW_DURATION + LINE_HOLD) {
      setLineProgress(1);
      setLineOpacity(1);
    } else if (cycleTime < LINE_TOTAL) {
      const fadeProgress = (cycleTime - LINE_DRAW_DURATION - LINE_HOLD) / LINE_FADE_DURATION;
      setLineProgress(1);
  ...
  const scale = size / Math.max(LOGO_WIDTH, LOGO_HEIGHT);
