...
export const PaginationControls = memo(
  ({ pageIndex, totalPages, onPageChange, pageSize, totalItems, onPageSizeChange }: PaginationControlsProps) => {
    const t = useTranslations('components.pagination');
    const currentPage = pageIndex + 1;
    const isFirstPage = pageIndex === 0;
    const isLastPage = pageIndex === totalPages - 1;
    const showPageSizeControls = pageSize != null && totalItems != null && onPageSizeChange != null;

    const startItem = pageSize ? pageIndex * pageSize + 1 : 0;
    const endItem = pageSize && totalItems ? Math.min(startItem + pageSize - 1, totalItems) : 0;
      ...
      <div className='flex flex-col gap-3 text-sm sm:flex-row sm:items-center sm:justify-between'>
        ...
        <nav
          ...
          className={cn(
            ...
            showPageSizeControls && 'border-border/40 sm:border-l sm:pl-4',
          )}
        >
          <Button
            variant='ghost'
