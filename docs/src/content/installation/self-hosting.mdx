...
```bash
ENABLE_GEOLOCATION=countries      # or "subdivisions" for region & city data
...
| `subdivisions` | Country + subdivision + city data (~61 MB) |

The `subdivisions` mode adds region and city tabs to the dashboard overview, providing granular geographic insights. Use `countries` if you only need country-level data and want to keep memory usage low.
