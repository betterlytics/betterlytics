...
```bash
ENABLE_GEOLOCATION=countries      # or "subdivisions" for regional drill-down
...
`ENABLE_GEOLOCATION` accepts three values:

| Value | Behavior |
|-------|----------|
| `false` (default) | Geolocation disabled |
| `countries` | Country-level geolocation (~9 MB) |
| `subdivisions` | Country + subdivision + city drill-down (~61 MB) |

The `subdivisions` mode enables clicking a country on the map to see a breakdown by state, province, or region, and also provides city-level data in the Top Cities tab on the overview page. Use `countries` if you only need country-level data and want to keep memory usage low. Note that the dashboard also uses more memory with `subdivisions` as it loads additional map geometry for regional boundaries.

> `true` is still accepted for backwards compatibility and behaves identically to `countries`.
...
When `ENABLE_GEOLOCATION=subdivisions` is set, both subdivision (state/province/region) and city data are extracted from the MaxMind GeoLite2-City database — the same database used for country lookups, just a larger version (~61 MB vs ~9 MB).
...
City-level data combined with other analytics dimensions (device, browser, time) can potentially identify individuals in small geographic areas — a risk known as "singling out" under GDPR Recital 26.

Betterlytics mitigates this with a **minimum visitor threshold** (default: 10). Geographic locations with fewer unique visitors than the threshold are hidden from the dashboard. This setting is configurable per-dashboard in **Settings > Data > Privacy**.

For low-traffic sites (fewer than ~1,000 monthly visitors), we recommend keeping the default threshold of 10 or setting it higher. See our [privacy decision document](https://github.com/betterlytics/betterlytics/blob/main/docs/GDPR_CITY_LEVEL_GEOGRAPHY.md) for the full analysis.
