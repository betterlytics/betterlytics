...
ENABLE_GEOSUBDIVISION=false   # set to "true" for region & city data
...
| `ENABLE_GEOSUBDIVISION` | `false` | Adds subdivision + city drill-down (~61 MB). Requires `ENABLE_GEOLOCATION=true` |
...
When `ENABLE_GEOSUBDIVISION=true` is set (alongside `ENABLE_GEOLOCATION=true`), both subdivision (state/province/region) and city data are extracted from the MaxMind GeoLite2-City database — the same database used for country lookups, just a larger version (~61 MB vs ~9 MB).
