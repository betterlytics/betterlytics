...
The self-hosted image is built from our stable releases, while Betterlytics Cloud always runs the latest version. New features launch on Cloud first and are included in self-hosted with a later image update, so the self-hosted image can temporarily lag behind what you see in the Cloud dashboard or elsewhere in these docs.
...
<Callout type="info">
Before relying on a specific feature, check the [cloud vs self-hosted comparison](/installation#cloud-vs-self-hosted) for its current self-hosted status.
</Callout>
