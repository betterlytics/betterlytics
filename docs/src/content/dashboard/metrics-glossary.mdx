...
**Definition:** The average amount of active time visitors spend viewing a specific page.

**How it's calculated:**
- Only counts time while the page is in focus - switching to another tab or app pauses the timer
- Includes exit pages and single-page sessions
...
Core Web Vitals are user‑centric performance metrics captured from real users (RUM) with the official `web-vitals` library. In the dashboard, they are summarized using percentiles (p50/p75/p90/p99); p75 is commonly used for pass/fail thresholds.

### LCP (Largest Contentful Paint)
**What it measures:** Loading performance – when the largest text or image becomes visible.
...
**What it measures:** Responsiveness – time from user interaction until the next visual update.
...
**What it measures:** When the first piece of content renders on screen.
...
**What it measures:** Server responsiveness – time from request start until the first byte is received.
...
### Composite score
**Definition:** A weighted blend (0–100) that summarizes multiple Web Vitals into a single value at each percentile (p50/p75/p90/p99).

- Each metric (LCP, INP, CLS, FCP, TTFB) is first mapped to a 0–100 score using the same Good/Fair/Poor thresholds shown above.
- We then compute a weighted average to prioritize user‑impacting metrics:
  - LCP: 0.30
  - INP: 0.30
  - CLS: 0.15
  - FCP: 0.15
...
Higher scores are better. This provides a quick way to compare performance across pages, devices, countries, browsers, and OS.

### Optimization opportunity
**Definition:** Estimated impact headroom for a row at each percentile, based on its traffic share and current composite score.

- Traffic share = samples for the row ÷ total samples on the page
- Opportunity ≈ traffic share × (100 − composite score)

Higher values indicate bigger potential impact if you improve that row’s performance.
...
- **Time measurements reflect active viewing:** Only time spent with the tab in focus is counted; backgrounded or idle time is excluded
