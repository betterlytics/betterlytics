...
description: "Monitor your website's uptime, response time, and SSL certificates with automated checks, email alerts, and Pushover push notifications."
---
...
Alerts notify you when your site goes down, recovers, or when SSL certificates are about to expire. You can enable or disable each alert type independently:
...
Email alerts are sent to the dashboard owner's email address.

<Callout type="info">
Email alerts require the SMTP / MailerSend integration to be configured on your instance. See the [self-hosting guide](/installation/self-hosting) for details.
</Callout>

#### Pushover Push Notifications

You can receive push notifications on your phone or desktop via [Pushover](https://pushover.net). This works alongside email alerts — you can use one or both.

**Setup:**

1. Create an account at [pushover.net](https://pushover.net)
2. Copy your **User Key** from the Pushover dashboard
3. Paste it into the **Pushover User Key** field in your monitor's alert settings

Each monitor can have its own Pushover user key, so different team members can receive notifications for different monitors.

<Callout type="info">
Pushover requires a server-side API token. Self-hosted users must set `ENABLE_PUSHOVER=true` and `PUSHOVER_API_TOKEN` in their environment. See the [self-hosting guide](/installation/self-hosting) for details.
</Callout>
...
- A **Down alert** is sent via email and/or Pushover (if enabled)
...
- A **Recovery alert** is sent via email and/or Pushover (if enabled)
