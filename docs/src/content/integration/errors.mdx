...
Betterlytics captures JavaScript errors from your site and sends them to your dashboard. Identical errors are automatically grouped so you can track unique issues over time.

## Quick start

### Using the script tag

Add `data-track-errors="true"` to your analytics script to start capturing errors.
...
| `data-replay-on-error` | boolean | `false` | Captures a replay when an error occurs, even for sessions not selected for regular recording. Requires `data-replay="true"` and consent. See [Error-triggered replay](#error-triggered-replay). |
...
When error tracking is combined with [Session Replay](/integration/session-replay), errors are automatically linked to replay recordings. You can jump from any error in the dashboard directly to the session replay to see what the user experienced and vice versa.
...
<Callout type="info">
Error-triggered recording requires replay consent. Recording only starts once consent is granted.
</Callout>
