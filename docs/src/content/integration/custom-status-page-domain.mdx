...
<Callout type="info">
Custom domains are available on **Professional** and **Enterprise** plans on Betterlytics Cloud. Self-hosted instances have no plan gate; see [Self-Hosted Instances](#self-hosted-instances) below.
</Callout>
...
- Confirm your plan includes custom domains (Professional or Enterprise, Cloud only)
...
The page remains reachable at `betterlytics.io/status/{your-slug}` as well, or at `https://<your-domain>/status/{your-slug}` on a self-hosted instance. With a custom domain configured, the custom domain is treated as the page's canonical address.

## Self-Hosted Instances

The in-app steps are the same: enter the domain in the status page's **Settings** tab and publish the page. The CNAME target is your instance's own hostname rather than `{your-slug}.status.betterlytics.io`; the editor shows the exact record.

| Type  | Name (host) | Value (target)          |
|-------|-------------|-------------------------|
| CNAME | `status`    | `analytics.example.com` |

In standalone mode the instance issues the certificate on the first visit after DNS resolves. Behind your own reverse proxy, the proxy terminates TLS for the hostname and forwards it with the `Host` header unchanged. Both setups are covered in [Custom Status Page Domains](/installation/self-hosting#custom-status-page-domains) in the Self-Hosting Guide.
