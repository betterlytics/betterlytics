...
| `data-scripts-base-url` | string (URL) | The analytics script's own origin | Base to load `replay.js` when replay is enabled. |
...
- Script loading: `replay.js` loads from the same origin as the analytics script by default. If you proxy `analytics.js` through your own domain, either proxy `replay.js` too or set `data-scripts-base-url` explicitly to your Betterlytics origin.
