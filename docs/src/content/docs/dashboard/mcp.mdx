...
Claude Desktop's **Connectors** UI only supports OAuth-based remote servers, so it can't pass a bearer token directly. Use the [`mcp-remote`](https://www.npmjs.com/package/mcp-remote) proxy instead (requires [Node.js](https://nodejs.org/) installed).

Open **Settings > Developer > Edit Config** to reveal `claude_desktop_config.json`, then add `betterlytics` to `mcpServers`.
...
```
  ...
  <Tabs.Tab>
Claude Desktop spawns `npx.cmd` through `cmd.exe /C` without quoting the path, which breaks on the space in `C:\Program Files\nodejs\`. Invoke `cmd` yourself instead so it can resolve `npx` via PATH:
...
After saving, fully quit Claude Desktop (tray icon > Quit on Windows, ⌘Q on macOS) and reopen it.
...
Add to your [MCP config](https://antigravity.google/docs/mcp) (`~/.gemini/antigravity/mcp_config.json`):
